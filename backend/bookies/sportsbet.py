...
def extract_match_element_type_1_odds(match_element):
    ...
    odds_elements = [button.find('span', attrs={'data-automation-id': 'price-text'}) for button in price_buttons]
    odds = [odd.get_text() for odd in odds_elements]
    if odds and 'SUS' not in odds:
...
def main(browser) -> list[BookieMatch]:
    ...
    for sport, url in config.items():
        ...
        for match_element in match_elements_type_1:
            ...
            date = match_element.find_previous('div', attrs={'data-automation-id': 'competition-event-group-title'}).text.replace(',', '')
            if "Today" in date or "Tomorrow" in date:
                date = standardise_today_tomorrow_date(date)
            time = match_element.find('span', attrs={'data-automation-id': 'competition-event-card-time'})
            time = re.match(r"(\d{1,2}:\d{2})", time.text).group(1) if time else datetime.now().time().strftime("%H:%M") # Sport is live if not time
            ...
            if team1_odds != 'SUS' and team2_odds != 'SUS': # Make sure betting isnt suspended
